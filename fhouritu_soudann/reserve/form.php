...
$booked_slots    = get_booked_slots($date, $type);
...
.slot-booked { display: inline-block; padding: 6px 14px; border: 2px solid #dee2e6;
  border-radius: 6px; font-size: .9rem; background: #f0f0f0; color: #aaa; cursor: not-allowed; }
.slot-blocked { display: inline-block; padding: 6px 14px; border: 2px solid #ffc107;
  border-radius: 6px; font-size: .9rem; background: #fff8e1; color: #856404; cursor: not-allowed; }
</style>
</head>
<body>
  ...
  <form method="post" action="">
    ...
    <div class="mb-3">
      ...
      <div class="d-flex flex-wrap gap-2">
        <?php foreach (TIME_SLOTS as $slot): ?>
          ...
          <div>
            <span class="slot-booked"><?= h($slot) ?>〜<br><small>予約済み</small></span>
          </div>
          <?php elseif (in_array($slot, $blocked_slots)): ?>
          <div>
            <span class="slot-blocked"><?= h($slot) ?>〜<br><small>予定あり</small></span>
          </div>
          <?php else: ?>
          <div class="slot-btn">
    ...
    <div class="mb-3">
      ...
      <input type="tel" class="form-control <?= isset($errors['tel']) ? 'is-invalid' : '' ?>"
             id="tel" name="tel" value="<?= h($old['tel'] ?? '') ?>" placeholder="例：090-0000-0000 または 09000000000">
      <div class="form-text">ハイフンあり・なしどちらでも入力できます。</div>
