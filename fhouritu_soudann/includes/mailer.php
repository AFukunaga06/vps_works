 ...
 * ２段階認証コードをメール送信
 */
function send_2fa_code_mail(string $to_email, string $code): bool {
    $mail = new PHPMailer(true);
    try {
        $mail->isSMTP();
        $mail->Host       = 'smtp.gmail.com';
        ...
        $mail->Subject = '【認証コード】' . SITE_NAME . ' ログイン確認';
 ...
 * パスワードリセットURLをメール送信
 */
function send_password_reset_mail(string $to_email, string $token): bool {
    $mail = new PHPMailer(true);
    try {
        $mail->isSMTP();
        $mail->Host       = 'smtp.gmail.com';
        ...
        $mail->Subject = '【パスワードリセット】' . SITE_NAME;
