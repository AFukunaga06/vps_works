...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['id'])) {
    ...
    if (in_array($action, $valid, true)) {
        ...
        // 確定時に相談者へ確定メールを送信
        if ($action === 'confirmed') {
            $r = $db->prepare("SELECT * FROM reservations WHERE id=?");
            $r->execute([$id]);
            $row = $r->fetch();
            if ($row) {
                send_confirmed_mail($row);
            }
        }
    }
...
<nav class="navbar admin-nav mb-4">
  <div class="container">
    ...
    <div class="d-flex gap-3 align-items-center">
      <a href="../crm/index.php" class="nav-link"><strong>弁護士CRM</strong></a>
      <a href="schedule.php" class="nav-link">スケジュール管理</a>
      ...
      <a href="settings.php" class="nav-link">設定</a>
...
<div class="container pb-5">
  ...
  <form method="get" class="row g-2 mb-4 align-items-end">
    ...
    <div class="col-auto ms-auto">
      <a href="export_csv.php?status=<?= urlencode($filter_status) ?>&type=<?= urlencode($filter_type) ?>&date=<?= urlencode($filter_date) ?>"
         class="btn btn-sm btn-success">CSVダウンロード</a>
    </div>
  </form>
