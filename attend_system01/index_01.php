...
function isSaturdayNonHoliday(string $d, array $holidays): bool {
    $dow = (int)date('w', strtotime($d)); // 0=日, 6=土
    if ($dow !== 6) return false;           // 土曜以外はNG
    if (in_array($d, $holidays)) return false; // 祝日の土曜はNG
    return true;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_member') {
    if (!hash_equals($csrf, $_POST['csrf'] ?? '')) {
        ...
        if ($modal_name === '') {
            ...
            header("Location: index_01.php?date=" . urlencode($date) . "&added=" . urlencode($modal_name));
            exit;
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_member') {
    ...
    header("Location: index_01.php?date=" . urlencode($redir_date) . "&deleted=1");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'verify_del') {
    ...
    if (($_POST['del_password'] ?? '') === DEL_PASSWORD) {
        ...
        header("Location: index_01.php?date=" . urlencode($redir_date) . $extra);
    } else {
        header("Location: index_01.php?date=" . urlencode($redir_date)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'bulk_delete') {
    ...
    header("Location: index_01.php?date=" . urlencode($redir_date) . "&bulk_deleted=" . $del_count);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'unconfirm') {
    ...
    header("Location: index_01.php?date=" . urlencode($post_date));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&
    ...
    $post_date = $_POST['date'] ?? $today;
    if ($post_date > $today) { header("Location: index_01.php?date=" . urlencode($post_date) . "&future_err=1"); exit; }

    // 土曜（祝日除く）チェック
    if (!isSaturdayNonHoliday($post_date, $jp_holidays)) {
        header("Location: index_01.php?date=" . urlencode($post_date) . "&weekday_err=1");
        exit;
    }
    ...
    header("Location: index_01.php?date=" . urlencode($post_date) . $param);
    exit;
}
...
$weekday_ok        = isSaturdayNonHoliday($date, $jp_holidays);
$weekday_err_msg   = $weekday_ok ? '' : '土曜日以外か祝日が選ばれました。';
$is_future  = $date > $today;
$future_msg = $is_future ? date('Y年n月j日', strtotime($date)) . 'はまだ来ていません。' : '';
?>
<!DOCTYPE html>
<html lang="ja">
<head>
...
<header>
  ...
  <nav>
    ...
    <a href="main_03.php" style="color:#fff;">メニューへ</a>
  </nav>
</header>
...
<form method="post" id="main-form">
  ...
  <div class="container">
    ...
    <div class="date-bar">
      ...
      <button type="button" class="btn-today"
              onclick="location.href='index_01.php?date=<?= $today ?>'">今日</button>
      <?php if ($is_future): ?>
        <span class="badge badge-deleted"><?= htmlspecialchars($future_msg) ?></span>
    ...
    <!-- 確定バー -->
    <?php if ($is_future): ?>
      <div class="confirm-bar" style="background:#fff3e0;border:1px solid #ffb74d;">
        <span style="font-size:1.2rem">⏳</span>
        <div class="confirm-info" style="color:#e65100;font-weight:bold;">
          <?= htmlspecialchars($future_msg) ?>
        </div>
      </div>
            ...
            <div class="member-row <?= $chk?'is-present':'' ?> <?= $is_confirmed?'':'editable' ?>">
              <label class="member-label <?= $is_confirmed?'':'editable' ?>">
                <input type="checkbox" name="present[]" value="<?= $m['id'] ?>"
                       <?= $chk          ? 'checked'  : '' ?>
                       <?= ($is_confirmed || $is_future) ? 'disabled' : 'onchange="updateRow(this)"' ?>>
  ...
  <div class="footer-bar">
    ...
    <?php elseif ($is_confirmed): ?>
      ...
      <?php else: ?>
        <button class="btn-save" type="submit" id="btn-save" <?= (!$weekday_ok || $is_future) ? 'disabled style="opacity:.45;cursor:not-allowed;"' : '' ?>>保存する</button>
      <?php endif; ?>
    </div>
    <div id="del-btns">
...
function isSaturdayNonHolidayJS(dateStr) {
  const d   = new Date(dateStr + 'T00:00:00');
  const dow = d.getDay();
  if (dow !== 6) return false;              // 土曜以外はNG
  if (jpHolidays.has(dateStr)) return false; // 祝日の土曜はNG
  return true;
}
function checkWeekday(dateStr) {
  ...
  if (!isSaturdayNonHolidayJS(dateStr)) {
    const label = '土曜日以外か祝日が選ばれました。';
...
function onDateChange(val) {
  checkWeekday(val);
  location.href = 'index_01.php?date=' + val;
}
...
function moveDate(delta) {
  ...
  location.href = 'index_01.php?date=' + y + '-' + m + '-' + day;
}
