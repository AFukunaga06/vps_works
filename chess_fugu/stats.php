...
try {
    ...
    foreach ($rows as $r) $totals[$r['result']] = (int)$r['c'];
    ...
    $recent = db()->query(
        "SELECT player_name, level, result, moves, end_reason, created_at
    ...
    $ranking = db()->query(
        "SELECT player_name,
                ...
                COUNT(*) AS total
         FROM games GROUP BY player_name
         ORDER BY wins DESC, total DESC LIMIT 10"
    )->fetchAll();
} catch (Throwable $ex) {
...
$reason_label = ['checkmate' => 'チェックメイト', 'stalemate' => 'ステイルメイト',
                 'threefold' => '三回同形', 'material' => '駒不足', 'resign' => '投了', 'draw' => '引分'];
?>
<!DOCTYPE html>
<html lang="ja">
<head>
...
<title>♟ fuguAI チェス ― 成績</title>
<style>
  * { box-sizing:border-box; }
  body { font-family:"Hiragino Sans","Yu Gothic",sans-serif; background:#1f2630; color:#fff; margin:0; padding:24px; }
  .wrap { max-width:720px; margin:0 auto; }
  h1 { font-size:24px; margin:0 0 4px; text-align:center; }
  .sub { color:#9fb6cf; font-size:13px; text-align:center; margin-bottom:20px; }
  a.btn { display:inline-block; background:#ffd34d; color:#1f2630; text-decoration:none;
  ...
  .card .l { font-size:12px; color:#9fb6cf; }
  .card.win .n { color:#7CFF9B; } .card.lose .n { color:#FF8F8F; } .card.draw .n { color:#ffe9a8; }
  ...
  th { background:rgba(255,255,255,.1); color:#9fb6cf; font-weight:normal; }
  ...
  .tag.win { background:#1f6b3a; color:#7CFF9B; } .tag.lose { background:#6b1f1f; color:#FF8F8F; }
  .tag.draw { background:#6b5e1f; color:#ffe9a8; }
  .name { text-align:left; }
  .empty { text-align:center; color:#9fb6cf; padding:20px; }
...
<div class="wrap">
  <h1>♟ fuguAI チェス ― 成績</h1>
  <div class="sub">あなた(白) vs AI(黒) の通算記録</div>
  ...
  <?php else: ?>
    <div class="cards">
      ...
      <div class="card lose"><div class="n"><?= $totals['lose'] ?></div><div class="l">AIの勝ち</div></div>
    ...
    <table>
      ...
      <tr>
        ...
        <td><?= $d['total'] ?></td><td><?= $d['win'] ?></td><td><?= $d['lose'] ?></td><td><?= $d['draw'] ?></td>
    ...
    <table>
      ...
      <tr>
        ...
        <td><?= (int)$r['wins'] ?></td><td><?= (int)$r['loses'] ?></td><td><?= (int)$r['draws'] ?></td>
        <td><?= (int)$r['total'] ?></td>
      </tr>
      <?php endforeach; ?>
    </table>
    <?php else: ?><p class="empty">まだ対戦記録がありません。</p><?php endif; ?>

    <h2>🕒 最近の対戦</h2>
    <?php if ($recent): ?>
    <table>
      <tr><th class="name">プレイヤー</th><th>難易度</th><th>結果</th><th>手数</th><th>決着</th><th>日時</th></tr>
      ...
      <tr>
        ...
        <td><?= (int)$g['moves'] ?></td>
        <td><?= $reason_label[$g['end_reason']] ?? e($g['end_reason']) ?></td>
        <td><?= e(date('n/j H:i', strtotime($g['created_at']))) ?></td>
      </tr>
      <?php endforeach; ?>
    </table>
    <?php else: ?><p class="empty">まだ対戦記録がありません。最初の一局を遊んでみましょう！</p><?php endif; ?>
  <?php endif; ?>
</div>
</body>
</html>
