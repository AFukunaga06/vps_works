...
export default function Checklist() {
  ...
  const getEntryValue = (itemId: number, dateStr: string) => {
    ...
    if (localEntries[key] !== undefined) return localEntries[key];
  ...
  const handleStatusChange = (itemId: number, dateStr: string, newStatus: string) => {
    const key = `${itemId}-${dateStr}`;
    setLocalEntries(prev => ({ ...prev, [key]: newStatus }));
  };
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            dateRange.forEach(date => {
                              ...
                              if (localEntries[key] !== undefined) {
                                updateEntryMutation.mutate({ itemId: item.id, date: dateStr, status: localEntries[key] }, {
                                  onSuccess: () => {
