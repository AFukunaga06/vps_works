...
// 評価変更
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'set_eval') {
    $id   = (int)($_POST['id'] ?? 0);
    $eval = $_POST['evaluation'] ?? '';
    if ($id > 0 && in_array($eval, ['A', 'B', 'C'])) {
        $pdo->prepare("UPDATE members SET evaluation=? WHERE id=?")->execute([$eval, $id]);
        $msg = "評価を更新しました。";
    }
}
...
$members = $pdo->query(
    "SELECT id, name, kana, group_name, is_active, COALESCE(evaluation,'B') AS evaluation FROM members ORDER BY group_name, kana, id"
)->fetchAll();
...
tbody tr.selected { background: #b3d9ff !important; }
.btn-sm { padding: 3px 10px; font-size: .8rem; border: none; border-radius: 4px; cursor: pointer; }
.btn-danger  { background: #e53935; color: #fff; }
.btn-success { background: #43a047; color: #fff; }
.sel-eval { padding: 2px 4px; font-size: .82rem; border: 1px solid #ccc; border-radius: 4px; cursor: pointer; }
</style>
</head>
<body>
<header>
...
<div class="container">
  ...
  <div class="list-columns">
    ...
    <div class="list-card list-<?= $cls ?>">
      ...
      <table>
        <thead><tr><th>#</th><th>氏名</th><th>フリガナ</th><th>評価</th><th></th></tr></thead>
        <tbody>
        <?php
        $i = 0;
        foreach ($list as $m):
          ...
          <tr class="<?= $m['is_active'] ? '' : 'inactive' ?>">
            ...
            <td>
              ...
              <form method="post" style="display:inline">
                <input type="hidden" name="action" value="set_eval">
                <input type="hidden" name="id" value="<?= $m['id'] ?>">
                <select name="evaluation" class="sel-eval" onchange="this.form.submit()">
                  <?php foreach (['A','B','C'] as $e): ?>
                    <option value="<?= $e ?>" <?= $m['evaluation']===$e?'selected':'' ?>><?= $e ?></option>
                  <?php endforeach; ?>
                </select>
              </form>
              <?php else: ?>
                <span style="font-size:.82rem;color:#aaa"><?= htmlspecialchars($m['evaluation']) ?></span>
              <?php endif; ?>
            </td>
            <td>
              <?php if ($m['is_active']): ?>
