...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_member') {
    if (!hash_equals($csrf, $_POST['csrf'] ?? '')) {
        ...
        if ($modal_name === '') {
            ...
            header("Location: index_01h.php?date=" . urlencode($date) . "&added=" . urlencode($modal_name));
            exit;
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_member') {
    ...
    header("Location: index_01h.php?date=" . urlencode($redir_date) . "&deleted=1");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'verify_del') {
    ...
    if (($_POST['del_password'] ?? '') === DEL_PASSWORD) {
        ...
        header("Location: index_01h.php?date=" . urlencode($redir_date) . $extra);
    } else {
        header("Location: index_01h.php?date=" . urlencode($redir_date)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'bulk_delete') {
    ...
    header("Location: index_01h.php?date=" . urlencode($redir_date) . "&bulk_deleted=" . $del_count);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'unconfirm') {
    ...
    header("Location: index_01h.php?date=" . urlencode($post_date));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&
    ...
    if (!isWeekdayNonHoliday($post_date, $jp_holidays)) {
        header("Location: index_01h.php?date=" . urlencode($post_date) . "&weekday_err=1");
        exit;
    }
    ...
    header("Location: index_01h.php?date=" . urlencode($post_date) . $param);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tc_action'])) {
    ...
    if ($mid > 0) {
        if ($_POST['tc_action'] === 'clock_clear') {
...
// ── 評価別時給 ────────────────────────────────────────────
$eval_rates = ['A' => 1300, 'B' => 1200, 'C' => 1100];
function evalRate(string $e, array $rates): int { return $rates[$e] ?? 1200; }
...
$men   = $pdo->query("SELECT id,name,COALESCE(evaluation,'B') AS evaluation FROM members WHERE is_active=1 AND group_name='男性' ORDER BY kana")->fetchAll();
$women = $pdo->query("SELECT id,name,COALESCE(evaluation,'B') AS evaluation FROM members WHERE is_active=1 AND group_name='女性' ORDER BY kana")->fetchAll();
...
function fmtWage(?int $min, int $rate): string {
  ...
  <div class="container">
    ...
    <div class="date-bar">
      ...
      <button type="button" class="btn-today"
              onclick="location.href='index_01h.php?date=<?= $today ?>'">今日</button>
      ...
      <div class="section sec-men">
        ...
        <div class="member-list" id="men-list">
          <?php foreach ($men as $m):
            ...
            $wh   = fmtWork($wm);
            $rate = evalRate($m['evaluation'], $eval_rates);
            $wage = fmtWage($wm, $rate);
          ?>
            <div class="member-row <?= $chk?'is-present':'' ?>">
              ...
              <span class="att-btns <?= $is_confirmed?'locked':'' ?>">
...
<div class="modal-overlay <?= $show_modal?'open':'' ?>" id="modal">
  <div class="modal">
    ...
    <?php endif; ?>
    <form method="post" action="index_01h.php?date=<?= urlencode($date) ?>">
...
function onDateChange(val) {
  checkWeekday(val);
  location.href = 'index_01h.php?date=' + val;
}
...
function moveDate(delta) {
  ...
  location.href = 'index_01h.php?date=' + y + '-' + m + '-' + day;
}
const TC_CSRF = '<?= htmlspecialchars($csrf) ?>';
const TC_DATE = '<?= htmlspecialchars($date) ?>';
const TC_ADM_PW = '1192';
const TC_RATES = {
<?php foreach (array_merge($men, $women) as $m): ?>
  <?= $m['id'] ?>: <?= evalRate($m['evaluation'], $eval_rates) ?>,
<?php endforeach; ?>
};
...
function calcWageJS(workStr, rate) {
  ...
  const wage = Math.round((h + m / 60) * rate);
...
function updateWorkHoursUI(id) {
  ...
  const rate = TC_RATES[id] || 1200;
  document.getElementById('wh-'+id).textContent   = wh;
  document.getElementById('wage-'+id).textContent = calcWageJS(wh, rate);
}
function tcPost(action, id, time) {
  return fetch('index_01h.php', {
    method: 'POST',
