...
<header>
  <div>
    ...
    <div class="sub">月〜金のみ / 月切替 / 男性・女性・男女合計 / 名簿CSV差し替え / 保存：この端末のブラウザ（localStorage）</div>
  </div>
...
const LS_ATT_PREFIX = "wk2026_att_v2:"; // wk2026_att_v2:YYYY-MM-DD:gender:name
...
function isWeekday(d){ const w=d.getDay(); return w>=1 && w<=5; } // Mon-Fri
...
prevBtn.addEventListener("click", ()=>{ state.month=Math.max(1,state.month-1); monthSel.value=String(state.month); render(); });
nextBtn.addEventListener("click", ()=>{ state.month=Math.min(12,state.month+1); monthSel.value=String(state.month); render(); });
monthSel.addEventListener("change", ()=>{ state.month=Number(monthSel.value); render(); });
...
scrollLeft.addEventListener("click", ()=> cardsEl.scrollBy({left:-900, behavior:"smooth"}));
scrollRight.addEventListener("click", ()=> cardsEl.scrollBy({left:900, behavior:"smooth"}));
...
function render(){
  ...
  for(const d of days){
    ...
    const badgeText =
      state.view==="male" ? `男性 ${dm}` :
      state.view==="female" ? `女性 ${df}` :
      `合計 ${dm+df}`;
    ...
    if(state.view==="male") addSection("男性名簿", "male", membersMale);
    else if(state.view==="female") addSection("女性名簿", "female", membersFemale);
    else{
