  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* ========================================
       ...
       ======================================== */
    .workday-panel{
      max-width:1200px;margin:0 auto;padding:0 16px 14px;
    }
    .workday-box{
    ...
    .workday-empty{
      color:var(--muted);font-size:13px;font-style:italic;padding:4px 0;
    }
    ...
    /* ========================================
       ...
       ======================================== */
    .filter-bar{
      max-width:1200px;margin:0 auto;padding:0 16px 10px;display:none;
    }
    ...
    .filter-inner .filter-date{
      font-weight:900;font-size:17px;color:var(--ink);
    }
    .filter-inner .filter-note{
    ...
    .twoCol .col table tr:last-child td{
      border-bottom:none;
    }
...
<header>
  ...
  <div>
    <a href="logout.php" class="logoutBtn">ログアウト</a>
  </div>
</header>
...
<div class="workday-panel">
  <div class="workday-box">
    ...
    <div class="workday-input-row">
      ...
      <button class="btn-confirm-workday" id="confirmWorkdayBtn">出勤日を確定</button>
    </div>
...
<div class="actions">
  ...
  <button class="ghost" id="clearBtn" title="この端末の記録だけ消します">この端末の記録をクリア</button>
</div>
 ...
 *  ■ 既存キー（変更なし・そのまま維持）
 *
 *    "wk2026_members_male_v2"
 *       → JSON配列 - 男性名簿メンバー一覧
 *       → 例: ["青木健太","石井直樹",...]
 *
 *    "wk2026_members_female_v2"
 *       → JSON配列 - 女性名簿メンバー一覧
 *       → 例: ["青木彩花","石井美咲",...]
 *
 *    "wk2026_att_v2:YYYY-MM-DD:gender:name"
 *       → 値: "1"（出席）or "0"（欠席）
 *       → gender = "male" | "female"
 *       → 例: "wk2026_att_v2:2026-02-16:male:青木健太" = "1"
 *       → ※ 出勤日フィルターで表示した際のチェックもこのキーに保存される。
 *         後日同じ日付を開くと同キーから復元されるため、
 *         チェックの入れ直し（変更）も即座にlocalStorageへ反映される。
 *
 *  ■ 追加キー
 *
 *    "wk2026_workdays"
 *       → JSON配列 - 確定した出勤日リスト（日付昇順ソート済み）
 *       → 例: ["2026-02-16","2026-03-05","2026-04-01"]
 *       → 重複なし
 *       → ページ再読込後も保持される
 *
 * =====================================================
...
const LS_WORKDAYS   = "wk2026_workdays";   /* ✅ 追加：確定出勤日リスト */
...
/* ========================================
   出席チェックの取得・設定（既存）
   ※ 出勤日フィルター表示でも同じキーを使用するため
     チェック状態の保存・復元は自動的に行われる
   ======================================== */
...
let state = {
  month: 1,
  view: "male",
  filterDate: null   /* ✅ null=通常表示 / "YYYY-MM-DD"=出勤日フィルター */
};
...
clearBtn.addEventListener("click", ()=>{
  if(!confirm("この端末の記録（チェック）を削除します。よろしいですか？")) return;
  const keysToRemove=[];
  for(let i=0;i<localStorage.length;i++){
    const k=localStorage.key(i);
    if(k && k.startsWith(LS_ATT_PREFIX)) keysToRemove.push(k);
  }
...
/* ========================================
   ...
   forceBoth パラメータ:
     true  = 出勤日フィルター表示時 → state.viewに関係なく男女両方を表示
     false = 通常月表示時 → state.viewに従って表示
   
   チェック状態の保持:
     getPresent() / setPresent() は既存のキー設計
     "wk2026_att_v2:YYYY-MM-DD:gender:name" を使用。
     フィルター表示でチェックを入れても同じキーに保存されるため、
     後日開いた際に自動的に復元される。
   ======================================== */
function buildCard(d, forceBoth){
  ...
  /* ========================================
     名簿行を生成するヘルパー（見出しなし・名前＋チェックのみ）
     チェック状態は既存キーから復元、変更は即localStorage保存
     ======================================== */
  function buildRows(tbl, gender, names){
    ...
    for(const name of names){
      ...
      tr.innerHTML = `
        <td>${name}</td>
        <td><input class="chk" type="checkbox" ${checked?"checked":""} /></td>
      `;
  ...
  /* ========================================
     forceBoth=true（指定日表示）→ 2カラムレイアウト
     forceBoth=false（通常月表示）→ 従来の1列テーブル
     ======================================== */
  if(forceBoth){
    /* ✅ 指定日モード：男女を左右2カラムで並列表示（見出しなし） */
...
function render(){
  ...
  if(state.filterDate){
    /* =====================================================
       ✅ フィルターモード：
       確定した出勤日のカードだけ表示
       → 男女両方の名簿を強制表示（forceBoth=true）
       → チェック状態は既存キーから自動復元
       → チェック変更は即座にlocalStorage保存
       ===================================================== */
...
/** 確定出勤日をlocalStorageから取得 */
function getWorkdays(){
  return lsGetJson(LS_WORKDAYS, []);
}

/** 確定出勤日をlocalStorageへ保存（昇順ソート） */
function saveWorkdays(arr){
  arr.sort();
  lsSetJson(LS_WORKDAYS, arr);
}

/**
 * 出勤日リストUI描画
 * 各日付を「M/D」形式のチップで横並び表示
 * [表示]ボタン / [×]ボタン / 日付クリックで名簿表示
 */
...
function removeWorkday(dateStr){
  ...
  if(!confirm(`出勤日 ${shortLabel}（${dateStr}）を削除しますか？`)) return;
...
function showDateFilter(dateStr){
  ...
  if(isWd){
    filterNote.textContent = "男性・女性 両方の名簿を表示 / チェックの入れ直しが可能です";
  } else {
    filterNote.textContent = "⚠️ この日は平日ではありません";
  }
...
workdayDateInput.addEventListener("keydown", (e)=>{
  if(e.key === "Enter") confirmWorkday();
});
backNormalBtn.addEventListener("click", ()=>{
  clearFilter();
  render();
});
