...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    ...
    if ($name !== '') {
        ...
        $msg = "「{$name}」を追加しました。";
        log_action($pdo, "会員追加", "【{$group}】{$name}（{$kana}）を追加");
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'deactivate') {
    $id = (int)($_POST['id'] ?? 0);
    if ($id > 0) {
        $name_row = $pdo->prepare("SELECT name FROM members WHERE id=?");
        $name_row->execute([$id]);
        $mname = $name_row->fetchColumn() ?: "ID:$id";
        $pdo->prepare("UPDATE members SET is_active=0 WHERE id=?")->execute([$id]);
        log_action($pdo, "退会処理", "{$mname} を退会");
        $msg = "退会処理を行いました。";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'activate') {
    $id = (int)($_POST['id'] ?? 0);
    if ($id > 0) {
        $name_row2 = $pdo->prepare("SELECT name FROM members WHERE id=?");
        $name_row2->execute([$id]);
        $mname2 = $name_row2->fetchColumn() ?: "ID:$id";
        $pdo->prepare("UPDATE members SET is_active=1 WHERE id=?")->execute([$id]);
        log_action($pdo, "復会処理", "{$mname2} を復会");
        $msg = "復会処理を行いました。";
    }
}
