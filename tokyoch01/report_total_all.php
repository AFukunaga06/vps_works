...
.summary-box { flex:1; min-width:100px; background:#f1f8e9; border-radius:8px; padding:12px; text-align:center; }
.summary-box .lbl { font-size:.75rem; color:#666; margin-bottom:3px; }
.summary-box .val { font-size:1.4rem; font-weight:bold; color:#2d6a3f; }
...
<body>
<header>
  <a class="site-title" href="main02.php">各集会総合計</a>
  <nav>
    <a href="report_total.php">礼拝合計</a>
    <a href="report_total02.php">祈祷会合計</a>
    <a href="report_total_chy.php">朝祈会合計</a>
...
<div class="container">
  ...
  <?php if ($mode === 'daily'): ?>
    ...
    foreach ($services as $key => $label) {
        $tbl = "attendance_{$key}";
        $stmt = $pdo->prepare("SELECT SUM(status='出席') AS present, SUM(status='欠席') AS absent FROM {$tbl} WHERE attend_date=?");
    ...
    ?>
    <div class="summary">
      <div class="summary-box"><div class="lbl">総合計出席</div><div class="val"><?= $total_present ?></div></div>
    ...
    <table>
      ...
      <?php endforeach; ?>
      <tr class="total-row"><td class="left">総合計</td><td style="color:#2e7d32"><?= $total_present ?></td><td></td></tr>
      </tbody>
    </table>
  ...
  <?php elseif ($mode === 'monthly'): ?>
    ...
    foreach ($services as $key => $label) {
        $tbl = "attendance_{$key}";
        $stmt = $pdo->prepare("SELECT SUM(status='出席') AS present, SUM(status='欠席') AS absent, COUNT(DISTINCT attend_date) AS days FROM {$tbl} WHERE DATE_FORMAT(attend_date,'%Y-%m')=?");
    ...
    ?>
    <div class="summary">
      <div class="summary-box"><div class="lbl">総合計出席延べ</div><div class="val"><?= $total_present ?></div></div>
      <?php foreach ($rows_by_service as $r): ?>
      <div class="summary-box"><div class="lbl"><?= $r['label'] ?></div><div class="val"><?= $r['present'] ?></div></div>
      <?php endforeach; ?>
    </div>
    <table>
      ...
      <?php endforeach; ?>
      <tr class="total-row"><td class="left">総合計</td><td></td><td style="color:#2e7d32"><?= $total_present ?></td><td></td></tr>
      </tbody>
    </table>
  ...
  <?php elseif ($mode === 'yearly'): ?>
    ...
    $table_data = [];
    foreach ($months as $ym) $table_data[$ym] = array_fill_keys(array_keys($services), 0);
    $grand_total = 0;
    foreach ($services as $key => $label) {
        $tbl = "attendance_{$key}";
        $stmt = $pdo->prepare("SELECT DATE_FORMAT(attend_date,'%Y-%m') AS ym, SUM(status='出席') AS present FROM {$tbl} WHERE YEAR(attend_date)=? GROUP BY ym");
    ...
    ?>
    <div class="summary">
      <div class="summary-box"><div class="lbl"><?= $target_year ?>年 総合計出席延べ</div><div class="val"><?= $grand_total ?></div></div>
    </div>
    <table>
      <thead>
        <tr>
          <th class="left">年月</th>
          <?php foreach ($services as $label): ?><th><?= $label ?></th><?php endforeach; ?>
          <th>合計</th>
        </tr>
      </thead>
      <tbody>
      <?php
      $year_totals = array_fill_keys(array_keys($services), 0);
      foreach ($table_data as $ym => $d):
        $sum = array_sum($d);
        foreach ($d as $k => $v) $year_totals[$k] += $v;
      ?>
        <tr>
          <td class="left"><a href="?mode=monthly&month=<?= $ym ?>" style="color:#2d6a3f"><?= $ym ?></a></td>
          <?php foreach ($d as $v): ?><td><?= $v ?: '-' ?></td><?php endforeach; ?>
          <td style="font-weight:bold"><?= $sum ?: '-' ?></td>
        </tr>
      <?php endforeach; ?>
      <tr class="total-row">
        <td class="left">合計</td>
        <?php foreach ($year_totals as $v): ?><td><?= $v ?></td><?php endforeach; ?>
        <td><?= array_sum($year_totals) ?></td>
      </tr>
      </tbody>
    </table>
  ...
  <?php else: // graph ?>
    ...
    for ($m = 1; $m <= 12; $m++) $months[] = sprintf('%d-%02d', $target_year, $m);
    $g_data = [];
    foreach (array_keys($services) as $key) $g_data[$key] = array_fill_keys($months, 0);
    foreach ($services as $key => $label) {
        $tbl = "attendance_{$key}";
        $stmt = $pdo->prepare("SELECT DATE_FORMAT(attend_date,'%Y-%m') AS ym, SUM(status='出席') AS present FROM {$tbl} WHERE YEAR(attend_date)=? GROUP BY ym");
        $stmt->execute([$target_year]);
        foreach ($stmt->fetchAll() as $r) {
            if (isset($g_data[$key][$r['ym']])) $g_data[$key][$r['ym']] = (int)$r['present'];
        }
    }
    $labels  = array_map(fn($ym) => ltrim(substr($ym,5),'0').'月', $months);
    $j_lbl   = json_encode($labels, JSON_UNESCAPED_UNICODE);
    $colors  = ['rgba(33,150,243,.75)','rgba(255,152,0,.75)','rgba(76,175,80,.75)','rgba(156,39,176,.75)','rgba(233,30,99,.75)','rgba(0,188,212,.75)','rgba(255,87,34,.75)'];
    $has_data = false;
    foreach ($g_data as $arr) { if (array_sum($arr) > 0) { $has_data = true; break; } }
    ?>
    <?php if (!$has_data): ?>
    ...
    new Chart(document.getElementById('allChart').getContext('2d'), {
      ...
      data: {
        labels: <?= $j_lbl ?>,
        datasets: [
          <?php $ci=0; foreach ($services as $key => $label): ?>
          { label: '<?= $label ?>', data: <?= json_encode(array_values($g_data[$key])) ?>, backgroundColor: '<?= $colors[$ci++] ?>' },
          <?php endforeach; ?>
        ]
      },
      options: {
        ...
        plugins: {
          legend: { position: 'top' },
          title: { display: true, text: '<?= $target_year ?>年　各集会月別出席数', font: { size: 15 }, padding: { bottom: 14 } }
        },
        scales: {
