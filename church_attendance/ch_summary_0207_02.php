...
<head>
  ...
  <title>日曜礼拝 出欠・人数集計（年収計つき）</title>
  <style>
    :root{
      --bg:#e7f6e7;
      --card:#ffffff;
...
function exportCsv(){
  ...
  const rows = [];
  rows.push(["区分","年","月","項目","男性","女性","合計"]);

  // 月集計（表示中）
  const mt = getCurrentMonthTotals();
  rows.push(["月集計", y, m, "合計", mt.male, mt.female, mt.total]);

  // 年集計（1〜12月 + 年間合計）
  const yt = getYearTotals(y);
  yt.months.forEach(mm=>{
    rows.push(["年集計", y, mm.monthIndex + 1, "月合計", mm.male, mm.female, mm.total]);
  });
  rows.push(["年集計", y, "", "年間合計", yt.male, yt.female, yt.total]);

  downloadCSV(`attendance_${y}.csv`, rows);
  alert("CSVを保存しました。");
}
