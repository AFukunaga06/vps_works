...
function isSunday(string $d): bool {
    return (int)date('w', strtotime($d)) === 0; // 0=日曜
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_member') {
    if (!hash_equals($csrf, $_POST['csrf'] ?? '')) {
        ...
        if ($modal_name === '') {
            ...
            $gender = ($modal_group === '男性') ? 'male' : 'female';
            $sort_stmt = $pdo->query("SELECT COALESCE(MAX(sort_order), 0) + 10 FROM roster");
            $next_sort = (int)$sort_stmt->fetchColumn();
            $pdo->prepare("INSERT INTO roster (name, gender, sort_order) VALUES (?, ?, ?)")
                ->execute([$modal_name, $gender, $next_sort]);
            header("Location: index_01_tr.php?date=" . urlencode($date) . "&added=" . urlencode($modal_name));
            exit;
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_member') {
    ...
    if (hash_equals($csrf, $_POST['csrf'] ?? '') && !empty($_SESSION['del_auth'])) {
        ...
        if ($del_id > 0) {
            $pdo->prepare("DELETE FROM attendance WHERE roster_id=?")->execute([$del_id]);
            $pdo->prepare("DELETE FROM roster WHERE id=?")->execute([$del_id]);
        }
    }
    header("Location: index_01_tr.php?date=" . urlencode($redir_date) . "&deleted=1");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'verify_del') {
    ...
    if (($_POST['del_password'] ?? '') === DEL_PASSWORD) {
        ...
        header("Location: index_01_tr.php?date=" . urlencode($redir_date) . $extra);
    } else {
        header("Location: index_01_tr.php?date=" . urlencode($redir_date)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'bulk_delete') {
    ...
    header("Location: index_01_tr.php?date=" . urlencode($redir_date) . "&bulk_deleted=" . $del_count);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'unconfirm') {
    ...
    header("Location: index_01_tr.php?date=" . urlencode($post_date));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&
    ...
    if (!isSunday($post_date)) {
        header("Location: index_01_tr.php?date=" . urlencode($post_date) . "&weekday_err=1");
        exit;
    }
    ...
    if ((int)$chk->fetchColumn() === 0) {
        ...
        $all_ids = $pdo->query("SELECT id FROM roster")
    ...
    header("Location: index_01_tr.php?date=" . urlencode($post_date) . $param);
    exit;
}
...
$men   = $pdo->query("SELECT id,name FROM roster WHERE gender='male' ORDER BY sort_order")->fetchAll();
$women = $pdo->query("SELECT id,name FROM roster WHERE gender='female' ORDER BY sort_order")->fetchAll();
...
$weekday_err_msg   = $weekday_ok ? '' : '日曜日が選ばれてません。';
?>
<!DOCTYPE html>
<html lang="ja">
<head>
...
/* CSV・バックアップボタン */
.btn-bar { display: flex; gap: 8px; flex-wrap: wrap; justify-content: flex-end; position: fixed; bottom: 16px; right: 16px; z-index: 50; }
.btn-csv-daily  { display:inline-block; padding: 10px 22px; background: #1e6f40; color: #fff; border: none; border-radius: 6px; font-size: .95rem; font-weight: bold; cursor: pointer; text-decoration: none; }
.btn-csv-daily:hover  { background: #155230; }
.btn-csv-month  { display:inline-block; padding: 10px 22px; background: #1e40af; color: #fff; border: none; border-radius: 6px; font-size: .95rem; font-weight: bold; cursor: pointer; text-decoration: none; }
.btn-csv-month:hover  { background: #1e3a8a; }
.btn-bk { display:inline-block; padding: 10px 22px; background: #6d4c41; color: #fff; border: none; border-radius: 6px; font-size: .95rem; font-weight: bold; cursor: pointer; text-decoration: none; }
.btn-bk:hover { background: #4e342e; }
/* セクション */
...
<header>
  <h1>通常日曜礼拝出席簿</h1>
  <nav>
    <a href="export_csv.php?type=daily&amp;date=<?= urlencode($date) ?>">日計CSV</a>
    <a href="export_csv.php?type=monthly&amp;year=<?= date('Y', strtotime($date)) ?>&amp;month=<?= date('n', strtotime($date)) ?>">月計CSV</a>
  ...
  <div class="container">
    ...
    <div class="date-bar">
      ...
      <button type="button" class="btn-today"
              onclick="location.href='index_01_tr.php?date=<?= $today ?>'">今日</button>
      <?php if ($weekday_err || !$weekday_ok): ?>
    ...
    <div class="columns">
      ...
      <div class="section sec-men">
        ...
        <div class="member-list" id="men-list">
          ...
          ?>
            <div class="member-row <?= $chk?'is-present':'' ?> <?= $is_confirmed?'':'editable' ?>">
              <label class="member-label <?= $is_confirmed?'':'editable' ?>">
                <input type="checkbox" name="present[]" value="<?= $m['id'] ?>"
                       <?= $chk          ? 'checked'  : '' ?>
                       <?= $is_confirmed ? 'disabled' : 'onchange="updateRow(this)"' ?>>
...
<div class="modal-overlay <?= $show_modal?'open':'' ?>" id="modal">
  <div class="modal">
    ...
    <?php endif; ?>
    <form method="post" action="index_01_tr.php?date=<?= urlencode($date) ?>">
...
function checkWeekday(dateStr) {
  ...
  if (!isSundayJS(dateStr)) {
    msgEl.textContent   = '日曜日が選ばれてません。';
    msgEl.style.display = '';
...
function onDateChange(val) {
  checkWeekday(val);
  location.href = 'index_01_tr.php?date=' + val;
}
...
function moveDate(delta) {
  ...
  location.href = 'index_01_tr.php?date=' + y + '-' + m + '-' + day;
}
