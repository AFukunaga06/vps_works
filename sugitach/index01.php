...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'unconfirm') {
    ...
    header("Location: index01.php?date=" . urlencode($post_date));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&
    ...
    header("Location: index01.php?date=" . urlencode($post_date) . $param);
    exit;
}
  ...
  <div class="container">
    <div class="date-bar">
      ...
      <input type="date" id="date-input" value="<?= htmlspecialchars($date) ?>"
             onchange="location.href='index01.php?date='+this.value">
      <button type="button" class="btn-nav" onclick="moveDate(1)">翌日 ▶</button>
      <button type="button" class="btn-today"
              onclick="location.href='index01.php?date=<?= $today ?>'">今日</button>
      <?php if ($saved): ?>
    ...
    <div class="columns">
      <div class="section sec-men">
        ...
        <div class="member-list" id="men-list">
          ...
          ?>
            <label class="member-row <?= $checked?'is-present':'' ?> <?= $is_confirmed?'':'editable' ?>">
...
function moveDate(delta) {
  ...
  location.href = 'index01.php?date=' + y + '-' + m + '-' + day;
}
