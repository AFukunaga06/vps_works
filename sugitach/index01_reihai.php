...
.date-bar input[type=date]:disabled { background: #f0f0f0; color: #888; border-color: #ccc; }
.btn-date-confirm { padding: 6px 16px; background: #1565c0; color: #fff; border: none; border-radius: 4px; cursor: pointer; font-size: .9rem; font-weight: bold; }
.btn-date-confirm:hover { background: #0d47a1; }
.btn-date-confirm.done { background: #4caf50; cursor: default; }
...
.btn-save:hover:not(:disabled) { background: #1e4e2c; }
...
.member-row input[type=checkbox]:disabled { opacity: 0.35; cursor: not-allowed; }
...
<form method="post" id="main-form">
  ...
  <div class="container">
    <div class="date-bar">
      ...
      <button type="button" id="btn-date-confirm" class="btn-date-confirm" onclick="confirmDate()">日付確定</button>
      <?php if ($saved): ?>
    ...
    <?php else: ?>
      <div class="confirm-bar is-unlocked">
        ...
        <button type="button" id="btn-confirm" class="btn-confirm" onclick="doConfirm()" disabled>✔ 確定する</button>
      </div>
    <?php endif; ?>
    ...
    <div class="columns">
      <div class="section sec-men">
        <div class="section-header">
          ...
          <span class="count">
            <span id="men-count"><?= $men_present ?></span>/<?= count($men) ?>名
              <button type="button" class="select-all" onclick="selectAll('men',false)" disabled>クリア</button>
          </span>
        </div>
        <div class="member-list" id="men-list">
          ...
          ?>
            <label class="member-row <?= $checked?'is-present':'' ?>">
              <input type="checkbox" name="present[]" value="<?= $m['id'] ?>"
                     <?= $checked ? 'checked' : '' ?>
                     onchange="updateRow(this)" disabled>
        ...
        <div class="section-header">
          ...
          <span class="count">
            <span id="women-count"><?= $women_present ?></span>/<?= count($women) ?>名
              <button type="button" class="select-all" onclick="selectAll('women',false)" disabled>クリア</button>
          </span>
        </div>
  ...
  <div class="footer-bar">
    <span class="total-info">
      ...
      <button class="btn-save" id="btn-save" type="submit" disabled>保存する</button>
  </div>
</form>

<script>
function confirmDate() {
  // チェックボックスを有効化・editable追加
  ...
  document.querySelectorAll('.select-all').forEach(btn => btn.disabled = false);
  // カレンダーを無効化
  document.getElementById('date-input').disabled = true;
  // 日付確定ボタンを確定済みに変更
  const btn = document.getElementById('btn-date-confirm');
  btn.textContent = '✓ 日付確定済';
...
function goDate(dateStr) {
  if (!isSunday(dateStr)) {
    if (!confirm(toJpDate(dateStr) + 'は日曜日ではありません。宜しいですか？')) {
      document.getElementById('date-input').value = '<?= htmlspecialchars($date) ?>';
      return;
    }
  }
