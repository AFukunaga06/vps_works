...
<div class="container py-4">
  ...
  <div class="row mb-3 g-3">
    <div class="col-md-6">
      <div class="card border-0 shadow-sm h-100" style="border-left: 4px solid #0d47a1 !important; border-left-width: 4px !important;">
        <div class="card-body">
          <h5 class="card-title" style="color:#0d47a1">一般相談</h5>
          <p class="card-text small mb-1">生活・仕事・IT・手続き・書類整理など</p>
          <p class="card-text small text-muted">初回無料（45分）／2回目以降 1,200円（税込）</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="card border-0 shadow-sm h-100" style="border-left: 4px solid #4a148c !important; border-left-width: 4px !important;">
        <div class="card-body">
          ...
          <p class="card-text small text-muted">初回無料（45分）／2回目以降 1,500円（税込）</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="d-flex gap-3 legend mb-2 flex-wrap">
    <span><span class="badge" style="background:#e3f2fd; color:#0d47a1">一般 ○枠</span> 一般相談の空き</span>
  ...
  <table class="calendar-table shadow-sm rounded">
    ...
    for ($d = 1; $d <= $days_in_month; $d++) {
        ...
        if ($bookable) {
            ...
            foreach (['一般相談' => 'general', '法律相談' => 'legal'] as $type => $cls) {
                $cnt = $avail[$date][$type] ?? 0;
                $short = ($type === '一般相談') ? '一般' : '法律';
                if ($cnt > 0) {
                    echo '<a href="../reserve/form.php?date=' . $date . '&type=' . urlencode($type) . '" class="' . $cls . '">'
                       . $short . ' ' . $cnt . '枠</a>';
                } else {
                    echo '<span class="full d-block px-1">' . $short . ' ×</span>';
                }
            }
            echo '</div>';
        } elseif ($date <= $today) {
