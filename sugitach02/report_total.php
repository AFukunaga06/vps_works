...
<body>
<header>
  <a class="site-title" href="main02.php">礼拝合計人数</a>
  <nav>
...
<div class="container">
  ...
  <?php elseif ($mode === 'yearly'): ?>
    ...
    $table_data = [];
    $grand_total = 0;
    foreach ($months as $ym) $table_data[$ym] = ['tr'=>0,'sr'=>0,'live'=>0];
    foreach ($services as $key => $label) {
        $tbl = "attendance_{$key}";
        $stmt = $pdo->prepare(
    ...
    <table>
      <thead><tr><th class="left">年月</th><th>通常礼拝</th><th>早朝礼拝</th><th>ライブ礼拝</th><th>合計</th></tr></thead>
      <tbody>
      <?php $y_tr=0;$y_sr=0;$y_live=0; foreach ($table_data as $ym => $d):
        $sum = $d['tr']+$d['sr']+$d['live']; $y_tr+=$d['tr'];$y_sr+=$d['sr'];$y_live+=$d['live']; ?>
        <tr>
      ...
      <tr class="total-row">
        <td class="left">合計</td>
        <td><?= $y_tr ?></td><td><?= $y_sr ?></td><td><?= $y_live ?></td>
        <td><?= $y_tr+$y_sr+$y_live ?></td>
      </tr>
      </tbody>
    </table>
  ...
  <?php else: // graph ?>
    ...
    $months = [];
    for ($m = 1; $m <= 12; $m++) $months[] = sprintf('%d-%02d', $target_year, $m);
    $g_tr = $g_sr = $g_live = array_fill_keys($months, 0);
    foreach ([['tr',&$g_tr],['sr',&$g_sr],['live',&$g_live]] as [$key, &$arr]) {
        $tbl = "attendance_{$key}";
        $stmt = $pdo->prepare(
            "SELECT DATE_FORMAT(attend_date,'%Y-%m') AS ym, SUM(status='出席') AS present
             FROM {$tbl} WHERE YEAR(attend_date)=? GROUP BY ym"
    ...
    $j_tr    = json_encode(array_values($g_tr));
    $j_sr    = json_encode(array_values($g_sr));
    $j_live  = json_encode(array_values($g_live));
    $has_data = array_sum($g_tr)+array_sum($g_sr)+array_sum($g_live) > 0;
    ?>
    <?php if (!$has_data): ?>
    ...
    new Chart(document.getElementById('totalChart').getContext('2d'), {
      ...
      data: {
        labels: <?= $j_lbl ?>,
        datasets: [
          { label: '通常礼拝', data: <?= $j_tr ?>, backgroundColor: 'rgba(33,150,243,0.75)' },
          { label: '早朝礼拝', data: <?= $j_sr ?>, backgroundColor: 'rgba(255,152,0,0.75)' },
          { label: 'ライブ礼拝', data: <?= $j_live ?>, backgroundColor: 'rgba(76,175,80,0.75)' },
        ]
      },
      options: {
        responsive: true,
        plugins: {
          legend: { position: 'top' },
          title: { display: true, text: '<?= $target_year ?>年　集会別月別出席数', font: { size: 15 }, padding: { bottom: 14 } }
        },
        scales: {
