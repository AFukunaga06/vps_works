...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_member') {
    if (!hash_equals($csrf, $_POST['csrf'] ?? '')) {
        ...
        if ($modal_name === '') {
            ...
            header("Location: index_sr02.php?date=" . urlencode($date) . "&added=" . urlencode($modal_name));
            exit;
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_member') {
    ...
    header("Location: index_sr02.php?date=" . urlencode($redir_date) . "&deleted=1");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'verify_del') {
    ...
    if (($_POST['del_password'] ?? '') === DEL_PASSWORD) {
        ...
        header("Location: index_sr02.php?date=" . urlencode($redir_date) . $extra);
    } else {
        header("Location: index_sr02.php?date=" . urlencode($redir_date)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'bulk_delete') {
    ...
    header("Location: index_sr02.php?date=" . urlencode($redir_date) . "&bulk_deleted=" . $del_count);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'unconfirm') {
    ...
    $pdo->prepare("DELETE FROM confirmed_dates_sr WHERE attend_date=?")->execute([$post_date]);
    log_action($pdo, "確定解除", "早朝礼拝: $post_date の確定を解除");
    header("Location: index_sr02.php?date=" . urlencode($post_date));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&
    ...
    if (!isSunday($post_date)) {
        header("Location: index_sr02.php?date=" . urlencode($post_date) . "&weekday_err=1");
        exit;
    }

    $chk = $pdo->prepare("SELECT COUNT(*) FROM confirmed_dates_sr WHERE attend_date=?");
    $chk->execute([$post_date]);
    if ((int)$chk->fetchColumn() === 0) {
        $checked = $_POST['present'] ?? [];
        $all_ids = $pdo->query("SELECT id FROM members WHERE is_active=1")
                       ->fetchAll(\PDO::FETCH_COLUMN);
        $pdo->prepare("DELETE FROM attendance_sr WHERE attend_date=?")->execute([$post_date]);
        $stmt = $pdo->prepare(
            "INSERT INTO attendance_sr (member_id, attend_date, status) VALUES (?, ?, ?)"
        );
    ...
    if ($_POST['action'] === 'confirm') {
        $pdo->prepare("INSERT IGNORE INTO confirmed_dates_sr (attend_date) VALUES (?)")
            ->execute([$post_date]);
    }

    $log_act = ($_POST['action'] === 'confirm') ? '確定保存' : '一時保存';
    $present_cnt = count($checked ?? []);
    log_action($pdo, $log_act, "早朝礼拝: $post_date 出席{$present_cnt}名");
    $param = ($_POST['action'] === 'confirm') ? '&confirmed=1' : '&saved=1';
    header("Location: index_sr02.php?date=" . urlencode($post_date) . $param);
    exit;
}

// ── 表示データ取得 ─────────────────────────────────────────
$chk = $pdo->prepare("SELECT confirmed_at FROM confirmed_dates_sr WHERE attend_date=?");
$chk->execute([$date]);
...
$att = $pdo->prepare("SELECT member_id FROM attendance_sr WHERE attend_date=? AND status='出席'");
$att->execute([$date]);
...
.section-total { padding: 8px 14px; font-size: .88rem; font-weight: bold; border-top: 1px solid #ddd; }
.sec-men .section-total { background: #e8f4ff; color: #1a5c99; }
.sec-women .section-total { background: #ffe8f4; color: #99195c; }
.grand-total { padding: 8px 14px; font-size: .95rem; font-weight: bold; background: #e8f5e9; border-top: 2px solid #aaa; color: #2d6a3f; }
...
<body>
<header>
  <h1 style="color:white;font-weight:bold;">（早朝7時から）</h1>
  <nav>
    <a href="index_sr02.php">出席入力</a>
    <a href="members.php">会員管理</a>
    <a href="report.php?m=sr">レポート</a>
  ...
  <div class="container">
    ...
    <div class="date-bar">
      ...
      <button type="button" class="btn-today"
              onclick="location.href='index_sr02.php?date=<?= $today ?>'">今日</button>
      <?php if ($future_err): ?>
    ...
    <div class="columns">
      ...
      <div class="section sec-men">
        ...
        <div class="section-total">男性合計：<span id="sum-men"><?= $men_present ?></span> / <?= count($men) ?>名</div>
      </div>
      ...
      <div class="section sec-women">
        ...
        <div class="section-total">女性合計：<span id="sum-women"><?= $women_present ?></span> / <?= count($women) ?>名</div>
        <div class="grand-total">全体合計：<span id="sum-total"><?= $men_present + $women_present ?></span> / <?= count($men) + count($women) ?>名</div>
      </div>
    </div>
  </div>
...
<div class="modal-overlay <?= $show_modal?'open':'' ?>" id="modal">
  <div class="modal">
    ...
    <?php endif; ?>
    <form method="post" action="index_sr02.php?date=<?= urlencode($date) ?>">
...
function onDateChange(val) {
  checkWeekday(val);
  location.href = 'index_sr02.php?date=' + val;
}
...
function moveDate(delta) {
  ...
  location.href = 'index_sr02.php?date=' + y + '-' + m + '-' + day;
}
