...
function isSunday(string $d): bool {
    return (int)date('w', strtotime($d)) === 6; // 6=土曜
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_member') {
    if (!hash_equals($csrf, $_POST['csrf'] ?? '')) {
        ...
        if ($modal_name === '') {
            ...
            header("Location: index_chy01.php?date=" . urlencode($date) . "&added=" . urlencode($modal_name));
            exit;
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_member') {
    ...
    header("Location: index_chy01.php?date=" . urlencode($redir_date) . "&deleted=1");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'verify_del') {
    ...
    if (($_POST['del_password'] ?? '') === DEL_PASSWORD) {
        ...
        header("Location: index_chy01.php?date=" . urlencode($redir_date) . $extra);
    } else {
        header("Location: index_chy01.php?date=" . urlencode($redir_date)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'bulk_delete') {
    ...
    header("Location: index_chy01.php?date=" . urlencode($redir_date) . "&bulk_deleted=" . $del_count);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'unconfirm') {
    ...
    $pdo->prepare("DELETE FROM confirmed_dates_chy01 WHERE attend_date=?")->execute([$post_date]);
    header("Location: index_chy01.php?date=" . urlencode($post_date));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&
    ...
    if (!isSunday($post_date)) {
        header("Location: index_chy01.php?date=" . urlencode($post_date) . "&weekday_err=1");
        exit;
    }

    $chk = $pdo->prepare("SELECT COUNT(*) FROM confirmed_dates_chy01 WHERE attend_date=?");
    $chk->execute([$post_date]);
    if ((int)$chk->fetchColumn() === 0) {
        $checked = $_POST['present'] ?? [];
        $all_ids = $pdo->query("SELECT id FROM members WHERE is_active=1")
                       ->fetchAll(\PDO::FETCH_COLUMN);
        $pdo->prepare("DELETE FROM attendance_chy01 WHERE attend_date=?")->execute([$post_date]);
        $stmt = $pdo->prepare(
            "INSERT INTO attendance_chy01 (member_id, attend_date, status) VALUES (?, ?, ?)"
        );
    ...
    if ($_POST['action'] === 'confirm') {
        $pdo->prepare("INSERT IGNORE INTO confirmed_dates_chy01 (attend_date) VALUES (?)")
            ->execute([$post_date]);
    }

    $log_act = ($_POST['action'] === 'confirm') ? '確定保存' : '一時保存';
    $present_cnt = count($checked ?? []);
    log_action($pdo, $log_act, "朝祈会: $post_date 出席{$present_cnt}名");
    $param = ($_POST['action'] === 'confirm') ? '&confirmed=1' : '&saved=1';
    header("Location: index_chy01.php?date=" . urlencode($post_date) . $param);
    exit;
}

// ── 表示データ取得 ─────────────────────────────────────────
$chk = $pdo->prepare("SELECT confirmed_at FROM confirmed_dates_chy01 WHERE attend_date=?");
$chk->execute([$date]);
...
$att = $pdo->prepare("SELECT member_id FROM attendance_chy01 WHERE attend_date=? AND status='出席'");
$att->execute([$date]);
...
$weekday_err_msg   = $weekday_ok ? '' : '土曜日が選ばれてません。';
...
<header>
  ...
  <nav>
    <a href="index_chy01.php">出席入力</a>
    <a href="members.php">会員管理</a>
    <a href="report.php?m=chy01">レポート</a>
  ...
  <div class="container">
    ...
    <div class="date-bar">
      ...
      <button type="button" class="btn-today"
              onclick="location.href='index_chy01.php?date=<?= $today ?>'">今日</button>
      <?php if ($future_err): ?>
...
<div class="modal-overlay <?= $show_modal?'open':'' ?>" id="modal">
  <div class="modal">
    ...
    <?php endif; ?>
    <form method="post" action="index_chy01.php?date=<?= urlencode($date) ?>">
...
function onDateChange(val) {
  checkWeekday(val);
  location.href = 'index_chy01.php?date=' + val;
}
...
function moveDate(delta) {
  ...
  location.href = 'index_chy01.php?date=' + y + '-' + m + '-' + day;
}
