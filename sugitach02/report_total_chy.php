...
<body>
<header>
  <a class="site-title" href="main02.php">朝祈会合計人数</a>
  <nav>
    <a href="index_chy01.php">朝祈会</a>
...
<div class="container">
  ...
  <?php if ($mode === 'daily'): ?>
    ...
    <?php
    $stmt = $pdo->prepare("SELECT SUM(status='出席') AS present, SUM(status='欠席') AS absent FROM attendance_chy01 WHERE attend_date=?");
    ...
    ?>
    <div class="summary">
      <div class="summary-box"><div class="lbl">出席</div><div class="val"><?= $present ?></div></div>
      <div class="summary-box"><div class="lbl">欠席</div><div class="val"><?= $absent ?></div></div>
    </div>
    <table>
      <thead><tr><th class="left">集会</th><th>出席数</th><th>欠席数</th></tr></thead>
      <tbody>
        <tr><td class="left">朝祈会</td><td style="color:#2e7d32"><?= $present ?></td><td style="color:#c62828"><?= $absent ?></td></tr>
      </tbody>
    </table>
  ...
  <?php elseif ($mode === 'monthly'): ?>
    ...
    <?php
    $stmt = $pdo->prepare("SELECT SUM(status='出席') AS present, SUM(status='欠席') AS absent, COUNT(DISTINCT attend_date) AS days FROM attendance_chy01 WHERE DATE_FORMAT(attend_date,'%Y-%m')=?");
    ...
    ?>
    <div class="summary">
      <div class="summary-box"><div class="lbl">出席延べ</div><div class="val"><?= $present ?></div></div>
      <div class="summary-box"><div class="lbl">記録日数</div><div class="val"><?= $days ?>日</div></div>
    </div>
    <table>
      <thead><tr><th class="left">集会</th><th>記録日数</th><th>出席延べ</th><th>欠席延べ</th></tr></thead>
      <tbody>
        <tr><td class="left">朝祈会</td><td><?= $days ?>日</td><td style="color:#2e7d32"><?= $present ?></td><td style="color:#c62828"><?= $absent ?></td></tr>
      </tbody>
    </table>
  ...
  <?php elseif ($mode === 'yearly'): ?>
    ...
    <?php
    $stmt = $pdo->prepare("SELECT DATE_FORMAT(attend_date,'%Y-%m') AS ym, SUM(status='出席') AS present FROM attendance_chy01 WHERE YEAR(attend_date)=? GROUP BY ym");
    $stmt->execute([$target_year]);
    $rows = $stmt->fetchAll();
    $months = [];
    for ($m = 1; $m <= 12; $m++) $months[sprintf('%d-%02d', $target_year, $m)] = 0;
    foreach ($rows as $r) { if (isset($months[$r['ym']])) $months[$r['ym']] = (int)$r['present']; }
    $grand_total = array_sum($months);
    ?>
    <div class="summary">
      <div class="summary-box"><div class="lbl"><?= $target_year ?>年 合計出席延べ</div><div class="val"><?= $grand_total ?></div></div>
    </div>
    <table>
      <thead><tr><th class="left">年月</th><th>出席延べ</th></tr></thead>
      <tbody>
      <?php foreach ($months as $ym => $present): ?>
        <tr>
          <td class="left"><a href="?mode=monthly&month=<?= $ym ?>" style="color:#2d6a3f"><?= $ym ?></a></td>
          <td><?= $present ?: '-' ?></td>
        </tr>
      <?php endforeach; ?>
      <tr class="total-row"><td class="left">合計</td><td><?= $grand_total ?></td></tr>
      </tbody>
    </table>
  ...
  <?php else: // graph ?>
    ...
    for ($m = 1; $m <= 12; $m++) $months[sprintf('%d-%02d', $target_year, $m)] = 0;
    foreach ($rows as $r) { if (isset($months[$r['ym']])) $months[$r['ym']] = (int)$r['present']; }
    $labels   = array_map(fn($ym) => ltrim(substr($ym,5),'0').'月', array_keys($months));
    $j_lbl    = json_encode($labels, JSON_UNESCAPED_UNICODE);
    $j_data   = json_encode(array_values($months));
    $has_data = array_sum($months) > 0;
    ?>
    <?php if (!$has_data): ?>
      <p class="no-data"><?= $target_year ?>年のデータはありません。</p>
    <?php else: ?>
    <div class="chart-wrap">
      <canvas id="chyChart"></canvas>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"></script>
    <script>
    new Chart(document.getElementById('chyChart').getContext('2d'), {
      type: 'bar',
      data: {
        labels: <?= $j_lbl ?>,
        datasets: [
          { label: '朝祈会', data: <?= $j_data ?>, backgroundColor: 'rgba(33,150,243,0.75)' }
        ]
      },
      options: {
        responsive: true,
        plugins: {
          legend: { position: 'top' },
          title: { display: true, text: '<?= $target_year ?>年　朝祈会月別出席数', font: { size: 15 }, padding: { bottom: 14 } }
        },
        scales: {
