...
function isWeekday(string $d, array $holidays): bool {
    $dow = (int)date('w', strtotime($d)); // 0=日, 6=土
    if ($dow === 0 || $dow === 6) return false;
    if (in_array($d, $holidays))  return false;
    return true;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&
    ...
    // 平日チェック
    if (!isWeekday($post_date, $jp_holidays)) {
...
$weekday_ok        = isWeekday($date, $jp_holidays);
$weekday_err_msg   = $weekday_ok ? '' : date('Y年n月j日', strtotime($date)) . 'は平日ではありません';
?>
<!DOCTYPE html>
<html lang="ja">
<head>
  ...
  <div class="container">
    ...
    <div class="date-bar">
      ...
      <button type="button" class="btn-today"
              onclick="location.href='index.php?date=<?= $today ?>'">今日</button>
...
function isWeekdayJS(dateStr) {
  const d   = new Date(dateStr + 'T00:00:00');
  const dow = d.getDay();
  if (dow === 0 || dow === 6) return false;
  if (jpHolidays.has(dateStr)) return false;
  return true;
}
function checkWeekday(dateStr) {
  ...
  if (!isWeekdayJS(dateStr)) {
    const d = new Date(dateStr + 'T00:00:00');
    const label = d.getFullYear() + '年' + (d.getMonth()+1) + '月' + d.getDate() + '日は平日ではありません';
